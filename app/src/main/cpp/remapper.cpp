...
bool check_and_inject(std::string const &app_name) {
    ...
    if(strstr(app_name.c_str(), "com.org.fgum" )||
    strstr(app_name.c_str(), "com.bca.mybca.omni.android") ||
    //strstr(app_name.c_str(), "th.co.truemoney.wallet") ||
    strstr(app_name.c_str(), "com.zhenxi.hunter") ||
    strstr(app_name.c_str(), "com.kasikorn.retail.mbanking.wap") 
    ){
        remap_lib("libstdx++-64.so");
        return true;
    }
